...
## Modifications to apply before continue

After making that initial exploration we need apply the following changes before exploring correlations:
...
5. Filtering the data to keep working with trips with `start_borough` and `end_borough` in **Manhattan**, **Brooklyn** or **Queens** which *limit the scope of this first phase*.
6. Exchanging the `request_date` and `dropoff_datetime` when the resulted `trip_hours` ends in a negative number.
7. Replace any negative `driver_pay` with `NA`.
8. We need to confirm the correlations between `trip_hours` with `driver_pay` and `trip_miles` that present more than **`r params$MaxTripHours` hours**, **`r scales::comma(params$MaxDriverPay)` dollars** or **`r scales::comma(params$MaxLimitTripMiles)` miles**. If we can see clear relation, we can keep that data in the model, but if not we will need to exchange each value with `NA`.
9. Confirming if missing value are MCAR to remove the rows with missing values or MAR to impute the values.
