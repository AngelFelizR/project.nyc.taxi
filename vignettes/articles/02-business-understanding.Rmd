...
- They work from 8 to 12 hours every day with 30 minutes breaks every 4 hours, after ending the current trip.
...
2. Restrict the location ids to use to the ones related to those zones.
...
```{r}
glue_safe("SELECT * FROM {ParquetSource} LIMIT 5") |> 
  dbGetQuery(conn = con) |>
  str()

glue_safe("SELECT COUNT(*) as nrow
          FROM {ParquetSource}
          WHERE PULocationID IN ({ValidZoneCodes}) AND
                DOLocationID IN ({ValidZoneCodes})") |> 
  dbGetQuery(conn = con) |>
  (\(df) df$nrow)() |>
  comma()
```
...
CREATE TEMP TABLE mean_distance AS
  WITH MeanDistance AS (
    ...
    FROM 
      {ParquetSource}
    WHERE
      PULocationID <> DOLocationID AND
      PULocationID IN ({ValidZoneCodes}) AND
      DOLocationID IN ({ValidZoneCodes})
    GROUP BY 
      PULocationID, 
      DOLocationID
...
dbExecute(conn = con, statement = MeanDistanceQuery)

dbGetQuery(conn = con,
           "SELECT * FROM mean_distance LIMIT 6")
```
