...
To gain a deeper understanding of the starting trips at the zone level, we'll create a **wrapper function**. This function will display the top 5 most significant zones and generate an interactive map. This map will illustrate the location of each zone in relation to the remaining locations within each borough.

> **Note**: *I've chosen not to add this function to the project package as it is quite specific and operates more effectively as a __non-pure function__.*

```{r class.source='fold-hide'}
print_zone_summary <- function(borough, prefix) {
  
  col_names <- paste0(prefix, c("zone", "long", "lat"))
  
  zone_dt <-
    TripsZoneDistribution[borough_col == borough,
                          .(count_M = sum(n/1e6)),
                          by = col_names,
                          env = list(borough_col = paste0(prefix, "borough"))]
  
  setorder(zone_dt, -count_M)
  
  zone_dt[, is_top_5 := "regular zone"]
  
  zone_dt[1:5, is_top_5 := "TOP 5"]
  
  print(zone_dt[is_top_5 == "TOP 5", !c("is_top_5")])
  
  zone_map <- plot_map(
    zone_dt,
    lng_var = col_names[2L],
    lat_var = col_names[3L],
    color_var = "is_top_5",
    color_palette = c("TOP 5" = "purple",
                      "regular zone" = BoroughColors[[borough]]),
    radius = 10,
    radius_var = "count_M",
    label_var = col_names[1L]
  )
...
In this borough, we can see that the most frequent locations to start a trip are concentrated **south of W 57th St**.

```{r}
print_zone_summary("Manhattan", "start_")
```

###### Brooklyn

```{r}
print_zone_summary("Brooklyn", "start_") |>
  leaflet::addMarkers(
    lat = 40.671207817400955, 
    lng = -73.9636514777047,
...
```{r}
print_zone_summary("Queens", "start_")
```
