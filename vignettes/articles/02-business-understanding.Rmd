...
2. Valid to be changed during the experiment.
- ***request_datetime***
- ***PULocationID***
- ***DOLocationID***
- *trip_miles*
...
- The taxis will start picking trips from their current zone, but every 3 minutes waiting they will be able to pick trips from the next closest zone and keep expanding the zones every 3 minutes until finding a trip.
...
LocationsMeanDistance <- glue("
  SELECT PULocationID,
         DOLocationID,
         mean(trip_miles) AS trip_miles_mean
  FROM {ParquetSource}
