...
Trips that takes longer than 3 hours doesn't look right and we should investigate more before adding that data to the model.
...
By checking the closer to the distribution we can confirm that 95% of trips present values from 5 to 50 dollars, but any fare higher that `r scales::comma(params$MaxDriverPay)` dollars seems weird won't be used to train the model.
...
## Transformation acctions to apply
...
5. As more of the trips start and end in Manhattan, Brooklyn or Queens, we are going to limit the model to predict on those zones.
6. Based on the distribution of `trip_miles` it doesn't make sense to train the models with trips longer than `r scales::comma(params$MaxLimitTripMiles)` miles so we will need to impute does values.
7. We need to omit any `driver_pay` with negative values or value higher than  `r scales::comma(params$MaxDriverPay)` dollars.

As we cannot investigate why this some observation present negative hours, we will assume that it was due to writing manually the date and we just need to exchange the `request_datetime` and `dropoff_datetime`.
