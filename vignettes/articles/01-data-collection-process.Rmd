...
```{r validate-parquet-files, include=FALSE}
...
To work with data large than RAM, we will need to create a simple data base with `duckdb` by following the next simple stops.

```{r}
ParquetSource2023 <-
  file.path(ParquetFolderPath, "year=2023") |>
...
```{r}
con <- DBI::dbConnect(duckdb::duckdb(), dbdir = here("my-db.duckdb"))
```
