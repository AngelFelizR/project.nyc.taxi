...
```{r}
dbGetQuery(con, "SELECT year, COUNT(*) AS n_rows FROM ValidZoneSample GROUP BY year")
```
...
WITH trip_data AS (
  ...
  INNER JOIN NycTrips t
    ...
    AND t.request_datetime <= vz.request_datetime + INTERVAL {TimeLimitMin} MINUTE
...
SELECT 
  vz.*,
  CASE 
    WHEN '{TakeTripCriteria}' = 'one_trip' THEN 
      CASE WHEN ad.best_performance <= vz.performance_per_hour THEN 1 ELSE 0 END
    WHEN '{TakeTripCriteria}' = 'median' THEN
      CASE WHEN ad.median_performance <= vz.performance_per_hour THEN 1 ELSE 0 END
    WHEN '{TakeTripCriteria}' = '75%' THEN
      CASE WHEN ad.percentile_75_performance <= vz.performance_per_hour THEN 1 ELSE 0 END
  END AS take_current_trip
FROM ValidZoneSample vz
