...
But even just taking the long trips we can see that 90% of those trips were shorter than **300 miles**, those trips doesn't look like good examples to train the model.
...
Due our limited compute power capacity we will need to down sample the data to be able to unlock all R capacities.
...
TrainingTrips <-
  ...
  lapply(sample_parquet,
         valid_zones = ZoneCodes[Borough %chin% BoroughToKeep, LocationID]) |>
  rbindlist()
```
...
TrainingTrips <- fst::read_fst("raw-data/TrainingTrips.fst",
                               as.data.table = TRUE)
```
...
TrainingTrips[, .N, .(same = PULocationID == DOLocationID)]

# Selecting variables to use
TrainingTrips[, join_zones(.SD,
                           zone_tb = ZoneCodes)
][,.(company = 
     ...
     start_borough,
     end_borough,
     same_borough = as.integer(start_borough == end_borough), 
     
     start_service_zone,
     end_service_zone,
     same_service_zone = as.integer(start_service_zone == end_service_zone),
     
     start_zone,
     end_zone,
     same_zone = as.integer(start_zone == end_zone),
