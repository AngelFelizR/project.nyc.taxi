...
- The taxis will start taking any trip around 3 miles from their current position, after 2 minutes they can start looking for trips around 5 miles and they cannot find any trip after 2 minutes they can expand the search **until 7 miles**.
...
4. Use the locations ids of all the trips we can estimate the mean distance between each pear of locations.

```{r parsermd-chunk-4}
# Define the SQL query
LocationsMeanDistance <- glue("
  SELECT 
    PULocationID,
    DOLocationID,
...
setDT(LocationsMeanDistance)

LocationsMeanDistance
```

5. Define a named vectors that can return the closest location id for all locations ids in the dataset. 
...
ClosestZone <-
  ...
  # We want to keep the original order of possible to avoid duplication
  )[, unique(.SD, by = c("PULocationID", "DOLocationID"))
  # We need to make sure to place the lower distances at the top  
  ][order(trip_miles_mean), 
    
    # This expression will select the first 15 rows of the data.table
    # for each PULocationID to avoid duplication
    .SD[1:15],
    keyby = "PULocationID"]


head(ClosestZone) |> str()
```

6. Restrict the location ids to use to the ones related to those zones.
