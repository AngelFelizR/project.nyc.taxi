...
- Only wheelchair-accessible vehicles can accept trips with that request.

- The TLC license number needs to keep constant for all trips in work day.
...
- They work from 8 to 12 hour every day with 30 minutes breaks every 4 hours.

- The taxis will find trips based on their time waiting and will take the first trip their valid radius:
    ...
    - 5-7 Minutes: Expand search to a 7-mile radius if no trip is found yet.

- As we cannot estimate the waiting time due high offer of taxis, so we will assume that **they will be able to accept the first requested trip**.


### Running trips simulation

1. Load the functions to use.
...
3. Check the number of rows available and the structure of the data.

```{r}
glue("SELECT COUNT(*) as nrow
      FROM {ParquetSource}") |> 
  dbGetQuery(conn = con) |>
  (\(df) df$nrow)() |>
  comma()

glue("SELECT * FROM {ParquetSource} LIMIT 5") |> 
  dbGetQuery(conn = con) |>
  str()
```


4. Listing all locations at 7 or less miles of each other, regardless or their original direction by placing the original direction over the opposite one.

```{r}
# Making the heavy work in SQL
MeanDistance7MilesQuery <- glue("
WITH MeanDistance AS(
  SELECT 
    PULocationID,
    DOLocationID,
...
# Transforming the data to data.table
MeanDistance7Miles <- dbGetQuery(conn = con, MeanDistance7MilesQuery)
setDT(MeanDistance7Miles)
MeanDistance7Miles
```

5. Removing duplicated distances by selecting the origin if we have it.

```{r}
ClosestLocations <- MeanDistance7Miles[
  # Arranging start point and ascending distance
  order(PULocationID, trip_miles_mean),
  
  # Selecting trips in original direction if we have it
  unique(.SD, by = c("PULocationID", "DOLocationID"))
]
```


. Restrict the location ids to use to the ones related to those zones.
...
setkeyv(ValidZoneCodes, "Borough")

ValidZoneCodes <-
  ValidZoneCodes[c("Manhattan", "Brooklyn", "Queens"), on = "Borough"]
```

7. Link all possible zones for each location id based on each Borough.
...
8. Defining when and where each simulation will start.
...
9. Based on each start point simulate a day of work.

```{r parsermd-chunk-9, eval = params$UpdateSimulation}
BaseLineSimulation <-
  RandomStartPoint[
...
```{r, include=FALSE, eval= params$UpdateSimulation}
usethis::use_data(BaseLineSimulation,
                  overwrite = TRUE,
                  compress = "xz",
                  version = 3)
```
