...
5. Calculate the mean distance present from one location to other if it has less than 7 miles.
...
dbGetQuery(conn = con,
           "SELECT * FROM MeanDistance LIMIT 6")
```

6. Sampling the data.

```{r}
StartPointQuary <- glue_safe("
SELECT *
FROM (
   SELECT 
      *,
...
StartPoint <- dbGetQuery(conn = con, StartPointQuary)
