...
- The TLC license number (taxi company) needs to keep constant for all trips in work day.

- Only wheelchair-accessible vehicles can accept trips with that request.

- They will take their last trip after working 8 hours.

- They have a 30 minutes break after 4 hours working once ending the current trip.

- The taxis will find trips based on their time waiting and will take the first trip in their valid radius:
...
3. Creating a object listing all parquet to query from.

```{r}
ParquetSource <-
  here("raw-data/trip-data/year=2023") |> 
...
4. Create a table listing all possible combinations of locations in Manhattan, Brooklyn and Queens, as they have more trips that other borough in NYC.
...
dbWriteTable(con, "ValidZoneCodes", ValidZoneCodes, temporary = TRUE)
```

4. Calculate the mean distance present from one location to other if it has less than 7 miles.

```{r}
MeanDistanceQuery <- glue_safe("
...
WITH MeanDistance AS (
  ...
  WHERE
    t1.PULocationID <> t1.DOLocationID
  GROUP BY 
    t1.PULocationID, 
    t1.DOLocationID
  HAVING 
...
5. Getting the starting trips to run the simulation based on random sample from valid zones in Manhattan, Brooklyn and Queens.

```{r eval= params$UpdateSimulation}
SimulationStartDayQuery <- glue_safe("
SELECT * 
FROM (
  SELECT t1.*
  FROM {ParquetSource} t1
