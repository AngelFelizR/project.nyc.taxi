...
```{r}
NycTrips2022 |>
  count_pct(trip_miles_status = case_when(trip_miles < 100 ~ "Normal trip",
                                          trip_miles >= 100 ~ "Long Trip"))
```

```{r}
NycTrips2022 |>
  filter(trip_miles >= 100) |>
  count_pct(trip_miles_status = case_when(trip_miles >= 600 ~ ">=600",
                                          trip_miles >= 500 ~ ">=500",
                                          trip_miles >= 400 ~ ">=400",
                                          trip_miles >= 300 ~ ">=300",
                                          trip_miles >= 200 ~ ">=200",
...
The proportion of problems is tiny.

```{r}
NycTrips2022 |>
  mutate(trip_minutes = arrow_minutes_between(request_datetime, dropoff_datetime)) |>
  count_pct(trip_minutes_status = case_when(trip_minutes < 0 ~ "Negative",
                                            trip_minutes > 120 ~ "Over 2 hours",
                                            TRUE ~ "Normal"))
```
