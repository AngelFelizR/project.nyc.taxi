...
```{r function-TrainingTrips, eval=FALSE, echo=FALSE}
#' TrainingTrips
#'
...
transform_for_correlation <- function(dt) {
  
  # Assert validation
  stopifnot("dt must be a data.table" = data.table::is.data.table(dt))
  
  # Copying the data to avoid transforming the original one
  dt <- data.table::copy(dt)
  
  # defining outcome to predict
  dt[, profit_rate := 
       (driver_pay + tips) / 
       as.double(difftime(dropoff_datetime,
                          request_datetime,
                          units = "hour"))]
...
summary(dt$profit_rate)

profit_rate[]

ggplot(dt, aes("",profit_rate))+
  geom_boxplot()
```
