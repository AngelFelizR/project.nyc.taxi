...
3. Create a connection with the parquet files without loading them into RAM.
...
MeanDistance <- dbGetQuery(con, MeanDistanceQuery)
...
5. Getting the starting trips to run the simulation.

```{r}
AllTripsQuery <- glue_safe("
SELECT 
...
StartDayQuery <- glue_safe("
SELECT * 
FROM ({AllTripsQuery}) 
USING SAMPLE reservoir(60 ROWS) REPEATABLE (3518);
")

StartDay <- dbGetQuery(con, StartDayQuery)
