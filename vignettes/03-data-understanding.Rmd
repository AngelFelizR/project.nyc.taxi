...
For most of the trips the dispatching number is **B03404** and **B03406**. The remaining codes represent a small proportion of observations.
...
Most of passengers don't agree to a shared/pooled ride.
...
It's really unusual for a passenger to request a wheelchair-accessible vehicle.
...
The number trips that take place in wheelchair-accessible vehicles is low but much higher that the number of trips requested with that capacity.
...
```{r}
TripsZoneDistribution |>
  count_pct(start_borough, start_service_zone, wt =  n)
```
...
```{r}
TripsZoneDistribution |>
  count_pct(end_borough, end_service_zone, wt =  n)
```
